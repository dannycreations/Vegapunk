...
vi.mock('@vegapunk/utilities', () => ({
  swap: vi.fn((arr: unknown[], a: number, b: number): void => {
    [arr[a], arr[b]] = [arr[b], arr[a]];
  }),
}));

vi.mock('@vegapunk/utilities/common', async (importOriginal: () => Promise<object>) => {
  const mod = await importOriginal();
  return {
    ...mod,
    isObjectLike: vi.fn((value: unknown): value is object => typeof value === 'object' && value !== null),
    merge: vi.fn((target: unknown, source: unknown): unknown => {
...
describe('Queue', () => {
  const numberMinComparator: Comparator<number> = (a: number, b: number): number => {
    return a - b;
  };
  ...
  const objectMinComparator: Comparator<Item> = (a: Item, b: Item): number => {
    return a.priority - b.priority;
  };

  const mixedTypeComparator: Comparator<number | Item> = (a: number | Item, b: number | Item): number => {
  ...
  describe('constructor', () => {
    ...
    it('should create an empty queue with the provided comparator', () => {
      const queue = new Queue<number>(numberMinComparator);
      expect(queue.size).toBe(0);
      expect(queue.last).toBeUndefined();
  ...
  describe('enqueue method', () => {
    ...
    it('should work with object types and a custom comparator', () => {
      const queue = new Queue<Item>(objectMinComparator);
  ...
  describe('find method', () => {
    ...
    it('should work with complex objects', () => {
      const objQueue = new Queue<Item>(objectMinComparator);
  ...
  describe('update method', () => {
    ...
    beforeEach(() => {
      queue = new Queue<number>(numberMinComparator);
      queue.enqueue(10, 5, 15, 3, 7, 12);
      vi.clearAllMocks();
    });
    ...
    it('should handle object merging if both old and new values are object-like', () => {
      ...
      expect(merge).toHaveBeenCalledWith(item2, updatedValue);
    ...
    it('should replace element if old value is primitive but new is object-like', () => {
      const numQueue = new Queue<number | Item>(mixedTypeComparator);
  ...
  describe('delete method', () => {
    ...
    it('should handle deleting from a queue with one element', () => {
      const singleQueue = new Queue<number>(numberMinComparator);
